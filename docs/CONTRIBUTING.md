...
- **Maintainer.** Ridgeway Lawrence — approves all PRs that touch `/brand/*.md`, `/tokens/*`, or `/logos/svg/*`.
...
| Editing logo SVGs in `/logos/svg/*` | Yes — Ridgeway |
| Regenerating PNGs / favicons from existing SVGs (`npm run build:favicons`) | No — but commit the outputs |
...
3. **Regenerate downstream assets** if you touched the generator or an SVG:
   - SVGs: `python3 scripts/generate_svgs.py`
   - PNGs + favicons: `python3 scripts/render_logo_pngs.py` (or `npm run build:favicons`)
...
The canonical logos are the **SVGs in `/logos/svg`**, generated by `scripts/generate_svgs.py`. Do not hand-edit an SVG if the intent can be expressed via the generator — modify the generator and regenerate all variants.

PNG and favicon exports are committed so downstream consumers don't need the toolchain, but they must be regenerated after any SVG change.
