...
> **Modern minimalist. Dark/light monochromatic. Accents are interactive, never decorative.** The single source of truth for any WRLD-branded interface, deck, mock, or asset.

**Owner:** WRLD Inc. (DBA WRLD Tech Co.) · EIN 84-5122446 · DUNS 03-999-5454 **Maintainer:** Ridgeway Lawrence — Lead Dev & Design | Founder **Source of truth:** [github.com/WRLDInc/DesignSystem](https://github.com/WRLDInc/DesignSystem)
...
- `assets/logos/wrld-mark-{master,black,white}.png` — starburst alone (original artwork).
- Lockups are a live-markup recipe — mark + Montserrat **WRLD** + uppercase sub-label (see the styleguide Logos section).
- `assets/logos/wrld-tech-{black,white}.png` — WRLD.TECH lockup with blue rule (original artwork).
- `assets/logos/favicons/` — full starburst favicon set: 16–512 PNGs, `favicon.ico`, `apple-touch-icon.png`, both `android-chrome-*.png` icons, and `site.webmanifest` (consumed by the styleguide, preview, and UI-kit pages; `deploy/index.html` uses the mirrored root `logos/favicons/` tree). Never the old circular disc.
...
React components compiled into `_ds_bundle.js` and exposed on `window.WRLDTechDesignSystemRemix_6276ea`. Import in card/DC HTML via `const { Button } = window.WRLDTechDesignSystemRemix_6276ea`.

**Shared**
- `Lockup` — canonical mark + wordmark + optional sub-brand label.

**wrld-tech** (marketing UI kit)
- `Header` — sticky nav with animated submenu + traveling underline.
- `Hero` — display headline + lede + CTA row.
- `Eyebrow` — uppercase letter-spaced label above headings.
- `Button` — primary / secondary / ghost / warm, hover-only accents.
- `ServicesGrid` — grid of sub-brand service cards.
- `ValuesStrip` — numbered values strip.
- `CTA` — closing call-to-action band.
- `Footer` — link columns + lockup + legal line.

**wrld-ai** (dashboard UI kit)
- `Sidebar` — dashboard left nav.
- `TopBar` — title + subtitle + primary action.
- `StatCard` — single KPI tile with signed delta.
