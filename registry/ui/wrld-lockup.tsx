import * as React from "react";

/**
 * WRLD Lockup — the authentic starburst mark, the Montserrat wordmark and an
 * optional right-aligned sub-brand label (TECH, AI, HOST, DESIGN …).
 *
 * Source of truth: github.com/WRLDInc/DesignSystem — ui_kits/_shared/Lockup.jsx
 * The mark is the original raster artwork, inlined at 128px (see below); the
 * retired parametric SVG set must never be reintroduced. Override `markSrc`
 * with the full-resolution file for large lockups.
 *
 * `theme` names the artwork, not the page: "dark" is the black mark for light
 * surfaces (default); "light" is the white mark for dark surfaces.
 */

/**
 * The mark. An inline 128px render of the authentic raster artwork
 * (registry/assets/wrld-mark-black-128.png, resized from
 * assets/logos/wrld-mark-black.png) so the component needs no network and
 * survives sandboxed previews — 21st's renderer refuses cross-origin requests.
 * The white variant for dark surfaces is the same pixels under filter: invert(1),
 * as the styleguide does. Sharp up to a wordmark size of about 40; for larger
 * lockups pass `markSrc` with the full-resolution file, e.g.
 * https://wrld.design/assets/logos/wrld-mark-black.png (or -white.png).
 */
const MARK_BLACK = "data:image/png;base64,";

const t = {
  mono950: "var(--wrld-mono-950, #0a0a0a)",
  mono0: "var(--wrld-mono-0, #ffffff)",
  mono600: "var(--wrld-mono-600, #52525b)",
  mono400: "var(--wrld-mono-400, #a1a1aa)",
  fontDisplay: "var(--wrld-font-display, Montserrat, 'Helvetica Neue', Arial, sans-serif)",
  ease: "var(--wrld-ease-standard, cubic-bezier(0.2, 0.8, 0.2, 1))",
} as const;

export interface WrldLockupProps {
  /** Sub-brand label shown to the right of the wordmark, e.g. "TECH". Null or undefined hides it. */
  sub?: string | null;
  /** "dark" = black mark on light surfaces (default); "light" = white mark on dark surfaces. */
  theme?: "dark" | "light";
  /** Wordmark font size in px; the mark scales relative to it. */
  size?: number;
  /** Animate the sub-label fade and the wordmark re-centre (nav lockups that morph on hover). */
  animated?: boolean;
  /** Custom mark image. Defaults to an inline 128px render of the authentic mark; pass https://wrld.design/assets/logos/wrld-mark-black.png (or -white) for large sizes. */
  markSrc?: string;
  style?: React.CSSProperties;
  className?: string;
}

export function WrldLockup({
  sub,
  theme = "dark",
  size = 24,
  animated = false,
  markSrc,
  style,
  className,
}: WrldLockupProps) {
  const src = markSrc ?? MARK_BLACK;
  const invertMark = !markSrc && theme === "light";
  const fg = theme === "light" ? t.mono0 : t.mono950;
  const subFg = theme === "light" ? t.mono400 : t.mono600;
  const hasSub = Boolean(sub);

  // Animated mode reserves a fixed text column (wordmark + sub line) and shifts
  // the wordmark up by half a sub-line when the sub appears, so it re-centres
  // around the mark instead of jumping.
  const subLineH = size * 0.5 + 4;
  const wordmarkY = animated && hasSub ? -subLineH / 2 : 0;

  return (
    <span
      className={className}
      style={{ display: "inline-flex", alignItems: "center", gap: 2, lineHeight: 1, ...style }}
    >
      <img
        src={src}
        alt="WRLD"
        width={size * 1.7}
        height={size * 1.7}
        style={{ height: size * 1.7, width: size * 1.7, objectFit: "contain", flexShrink: 0, display: "block", filter: invertMark ? "invert(1)" : undefined }}
      />
      <span
        style={{
          position: "relative",
          display: "inline-flex",
          flexDirection: "column",
          paddingRight: "4.2em",
          lineHeight: 1,
          height: animated ? size + subLineH : "auto",
          justifyContent: animated ? "flex-start" : "center",
        }}
      >
        <span
          style={{
            fontFamily: t.fontDisplay,
            fontWeight: 700,
            fontSize: size,
            letterSpacing: "0.04em",
            color: fg,
            transform: `translateY(${wordmarkY}px)`,
            transition: animated ? `transform 280ms ${t.ease}` : "none",
            marginTop: animated ? subLineH / 2 : 0,
          }}
        >
          WRLD
        </span>
        <span
          aria-hidden={!hasSub}
          style={{
            fontFamily: t.fontDisplay,
            fontWeight: 500,
            fontSize: size * 0.5,
            letterSpacing: "0.18em",
            textTransform: "uppercase",
            color: subFg,
            marginTop: 4,
            opacity: hasSub ? 1 : 0,
            transform: animated ? `translateY(${hasSub ? 0 : -4}px)` : "none",
            transition: animated ? `opacity 220ms ease, transform 280ms ${t.ease}` : "none",
            height: animated ? subLineH : hasSub ? "auto" : 0,
            display: animated || hasSub ? "block" : "none",
            pointerEvents: "none",
          }}
        >
          {sub || ""}
        </span>
      </span>
    </span>
  );
}

export default WrldLockup;
