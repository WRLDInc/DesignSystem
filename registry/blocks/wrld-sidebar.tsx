...
/**
 * WRLD Sidebar — the WRLD.AI dashboard left navigation: lockup, a "Workspace"
 * eyebrow, icon links and a plan footer.
 *
 * Source of truth: github.com/WRLDInc/DesignSystem — ui_kits/wrld-ai/Sidebar.jsx
 * Icons come from Lucide, the working icon set of the system (1.5px stroke,
 * currentColor). The lockup is inlined because 21st publishes one self-contained
 * file per component; the full version is wrld-lockup in the same library.
 * Links are real buttons, so the nav is keyboard reachable.
 */
...
export interface WrldSidebarItem {
  id: string;
  label: string;
  /** Lucide icon name (e.g. "bot", "activity") or a React node. */
  icon?: string | React.ReactNode;
}

export interface WrldSidebarProps extends React.HTMLAttributes<HTMLElement> {
  items?: WrldSidebarItem[];
  /** Active item id. */
  active?: string;
  onNavigate: (id: string) => void;
  /** Sub-brand label on the lockup. Defaults to "AI". */
  sub?: string | null;
  ...
  markSrc?: string;
  /** Eyebrow above the links. Defaults to "Workspace". */
  sectionLabel?: React.ReactNode;
  /** Footer: workspace name and plan line. Pass null to hide. */
  workspace?: { name: string; plan?: string } | null;
}

export const WRLD_SIDEBAR_ITEMS: WrldSidebarItem[] = [
...
function ItemIcon({ icon, size = 16 }: { icon?: string | React.ReactNode; size?: number }) {
  if (icon == null) return null;
  if (typeof icon !== "string") return <>{icon}</>;
  const Icon = (icons as Record<string, LucideIcon>)[toPascal(icon)] ?? icons.Circle;
...
export function WrldSidebar({
  items = WRLD_SIDEBAR_ITEMS,
  active,
  onNavigate,
  sub = "AI",
  markTheme = "dark",
  ...
  workspace = { name: "Acme Roofing", plan: "Pro plan · 12 agents" },
  style,
  ...rest
}: WrldSidebarProps) {
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "4px 8px", marginBottom: 20 }}>
        <Lockup sub={sub} theme={markTheme} size={16} markSrc={markSrc} />
      </div>
      {sectionLabel && (
        <div
          className="eyebrow"
      ...
      <nav aria-label="Workspace" style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        {items.map((it) => {
      ...
      {workspace && (
        <div style={{ marginTop: "auto", padding: "12px 10px", borderTop: `1px solid ${t.border}`, fontSize: 12, color: t.fgMuted }}>
          <div style={{ fontWeight: 500, color: t.fg }}>{workspace.name}</div>
          {workspace.plan && <div style={{ fontFamily: t.fontMono, fontSize: 11 }}>{workspace.plan}</div>}
        </div>
      )}
    </aside>
  );
