...
/**
 * WRLD Footer — site footer with the lockup, a short description, link columns
 * and a legal line in mono.
 *
 * Source of truth: github.com/WRLDInc/DesignSystem — ui_kits/wrld-tech/Footer.jsx
 * The lockup is inlined (static variant) because 21st publishes one
 * self-contained file per component; the full lockup is wrld-lockup in the
 * same library. The mark is the authentic raster artwork, inlined (see below).
 */
...
export interface WrldFooterLink {
  label: string;
  href?: string;
}

export interface WrldFooterColumn {
  title: string;
  items: WrldFooterLink[];
}

export interface WrldFooterProps extends React.HTMLAttributes<HTMLElement> {
  /** Sub-brand label on the lockup, e.g. "TECH". Omit for the plain wordmark. */
  sub?: string | null;
  ...
  columns?: WrldFooterColumn[];
  /** Left side of the legal line. */
  legal?: React.ReactNode;
  /** Right side of the legal line, e.g. a version. */
  meta?: React.ReactNode;
}

export const WRLD_FOOTER_COLUMNS: WrldFooterColumn[] = [
  {
    title: "Services",
    items: [
  ...
  {
    title: "Company",
    items: [{ label: "About" }, { label: "Values" }, { label: "Contact" }, { label: "Careers" }],
  },
  {
    title: "Resources",
    items: [{ label: "Client portal" }, { label: "Status" }, { label: "Brand kit", href: "https://wrld.design" }, { label: "Privacy" }],
  },
];
...
function FooterLink({ label, href }: WrldFooterLink) {
  ...
  return (
    <a
      ...
      style={{
        fontSize: 13,
        color: hover ? t.accentPrimary : t.fg,
        textDecoration: "none",
...
export function WrldFooter({
  ...
  return (
    <footer
      ...
      {...rest}
    >
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 48, marginBottom: 32 }}>
        <div style={{ gridColumn: "span 1" }}>
          <div style={{ marginBottom: 16 }}>
            <Lockup sub={sub} theme={markTheme} size={20} markSrc={markSrc} />
          </div>
          <div style={{ fontSize: 13, color: t.fgMuted, maxWidth: 320, lineHeight: 1.55 }}>{description}</div>
        </div>
        {columns.map((c) => (
          <div key={c.title}>
            <div
